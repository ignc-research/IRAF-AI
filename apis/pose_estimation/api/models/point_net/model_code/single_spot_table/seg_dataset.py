...
def load_h5(h5_filename):
    ...
    data = f['data'][:]
    label = f['label'][:]
    # print "load_h5:",data.shape
    # print "load_h5:",label.shape
    return (data, label)
def loadDataFile(filename):
    return load_h5(filename)


class SegH5Dataset(object):
    ...
    def _augment_batch_data(self, batch_data, batch_label):
        ...
        return provider.shuffle_points_lable(rotated_data,batch_label)
    ...
    def _load_data_file(self, filename):
        ...
        # print self.current_label.shape
        self.current_label = np.squeeze(self.current_label,axis=None)
        # print self.current_label.shape
        self.batch_idx = 0
        if self.shuffle:
    ...
    def next_batch(self, augment=False):
        ...
        label_batch = self.current_label[start_idx:end_idx, 0:self.npoints].copy()
        self.batch_idx += 1
        if augment: data_batch, label_batch = self._augment_batch_data(data_batch, label_batch)
        return data_batch, label_batch

if __name__=='__main__':
    d = SegH5Dataset('pcd/dataset/train.txt',4,1024)
    print(d.shuffle)
    print(d.has_next_batch())
    ps_batch, seg_batch = d.next_batch(True)
    print(ps_batch.shape)
    print(seg_batch.shape)
    data = np.c_[ps_batch[0],seg_batch[0]]
    print data.shape
    # seg_makeh5.points2pcd('ttt.pcd',data)
    # print np.max(ps_batch)
    # print(np.min(ps_batch))
    # print ps_batch
    # print cls_batch
