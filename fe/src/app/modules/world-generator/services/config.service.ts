...
import { ConfigEnvNode, exportEnvironment, parseEnvironment } from 'src/app/models/config/config-env-node';
...
import { ConfigRobotNode, exportRobot, parseRobot } from 'src/app/models/config/config-robot-node';
import { ConfigSensorNode, exportSensor, parseSensor } from 'src/app/models/config/config-sensor-node';
import { SceneService } from './scene.service';
import { UiControlService } from './ui-control.service';
import { ConfigGoalNode, exportGoal, parseGoal } from 'src/app/models/config/config-goal-node';
import { Goal } from 'src/app/models/goal';
import { Environment } from 'src/app/models/environment';
import { ConfigObstacleNode, exportObstacle, parseObstacles } from 'src/app/models/config/config-obstacle-node';
...
export class ConfigService {
  ...
  exportConfig() {
    ...
    const bbox = new THREE.Box3().setFromObject(this.sceneService.rootNode.ref.value);
    env.world.config.workspace_boundaries = [bbox.min.x, bbox.max.x, bbox.min.y, bbox.max.y, bbox.min.z, bbox.max.z];

    this.sceneService.robotGroup?.children.map(x => x as Robot).forEach(robot => {
      const configRobot = exportRobot(robot);
      configRobot.sensors = robot.children.filter(x => x instanceof Sensor).map(x => exportSensor(x as Sensor));
    ...
    env.world.config.obstacles = this.sceneService.obstacleGroup?.children.map(x => exportObstacle(x as Obstacle)) ?? [];
  ...
  parseConfig(configStr?: string) {
    ...
    const rootNode = parseEnvironment(this.apiService.environment!, data?.env);
    ...
    data.env.robots.forEach(robotNode => {
      ...
      const parsedSensors = robotNode.sensors.map(sensorNode => parseSensor(this.apiService.sensors, sensorNode));
    ...
    data.env.world.config.obstacles.forEach(obstacleNode => {
      const obstacle = parseObstacles(this.apiService.obstacles, obstacleNode);
      ConfigUtils.parseTrajectories(obstacle.params ?? {}).forEach(trj => obstacle.addChild(trj));
