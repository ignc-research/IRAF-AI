...
import { WeldingSeamFrame } from '../ConfigHelper';
...
export class TorchAnimation {
    ...
    animationSpeed;
    lastUpdate = Date.now();
    cb: (animId: number) => void;
    state = 1;
    steps: number[] = []
    duration = 0
    animId: number = -2;

    getDistance = (to: THREE.Vector3, from: THREE.Vector3) => new THREE.Vector3(from.x, from.y, from.z).distanceTo(new THREE.Vector3(to.x, to.y, to.z));

    constructor(from: WeldingSeamFrame, to: WeldingSeamFrame, torch: THREE.Object3D, cb: (animId: number) => void, animationSpeed = 1) {
    ...
    static getVector = (vector: THREE.Vector3) => new THREE.Vector3(vector.x, vector.y, vector.z).normalize().multiplyScalar(AppSettings.scaleFactor);

    static getMatrix = (point: WeldingSeamFrame) => {
