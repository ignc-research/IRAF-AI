...
export default function PredictDialog(props: { isOpen: boolean, setIsOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
    ...
    const onSave = async () => {
        try {
            ...
            const groundTruth = Object.keys(tabContext.layers).find(x => tabContext.layers[x].type == 'GroundTruth');
            if (!groundTruth) {
                ToastHelper.error('There is no existing ground truth for the prediction!');
                return;
            }
            if (!tabContext.weldingElement.obj || !tabContext.weldingElement.mtl) {
                ToastHelper.error('Either the welding obj file or the welding mtl file is missing!');
                return;
            }
            const startTime = Date.now();
            const model = {...selectedModel};

            const result = await new PoseEstimationApi().predict(model, tabContext.weldingElement.obj, tabContext.layers[groundTruth].xml, tabContext.weldingElement.mtl);

            model.prediction_duration = Date.now() - startTime;

            const addResult = await ConfigHelper.addLayers(result, model, tabContext.welds, tabContext.layers);

            tabContext.setLayers(addResult.layers);
