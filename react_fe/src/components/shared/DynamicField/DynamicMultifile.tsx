...
import FileSelector, { SelectedFile } from "../FileSelector";
import { Icon } from '@iconify/react';
export default function DynamicMultifile(props: { model: any, onChange: (val: string) => void }) {
    const model = props.model;
    const [state, setState] = useState<SelectedFile[]>([]);

    const onSelect = (files: SelectedFile[]) => {
        setState(files);
    }

    const onRemoveFile = (file: SelectedFile) => {
    ...
    const fileToString = (files: SelectedFile[]) => JSON.stringify(files);
    return (
